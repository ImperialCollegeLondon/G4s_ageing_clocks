...
5. Get number of all CpGs in the genome and their overlaps with the G4 dataset. The output file will contain data on how many global CpGs overlap with G4s. This will be used later by other functions to obtain the enrichment value of G4s relative to global CpGs instead of relative to random regions in the genome. The `$all_CpGs_file.bed` containing the coordinates of all CpGs in the genome needs to be generated beforehand [see example below](#global-cpg).  The file `$genome_file` is a text file characterising valid regions of the genome. This step has a long execution time.
```
...
<h3 id="global-cpg">Obtaining global CpG sites</h3>
